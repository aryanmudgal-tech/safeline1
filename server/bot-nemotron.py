...
"""Safeline — police documentation voice agent (NVIDIA Nemotron variant).

An officer calls in after an incident and narrates what happened. The bot
listens, asks targeted follow-up questions to fill required report fields, then
generates the appropriate police reports and texts the officer a review link.

Pipeline: Nemotron Speech Streaming STT → Nemotron-3-Super-120B LLM → Gradium
TTS, with direct function tools registered on the LLM context. The pipeline,
Twilio transport, and Pipecat Cloud deploy are unchanged from the starter kit —
only the business logic (system prompt + tools) has been swapped, in
``police_agent.py``.
...
async def run_bot(
    ...
    Args:
        ...
        from_number: Caller's phone number (Twilio path only), used to text the
            officer the review link once reports are generated.
    ...
    """
    logger.info("Starting Safeline bot")

    # Per-call session state, closed over by the tool functions. ``context`` is
    # filled in after the LLM context is created below so the report-generation
    # tool can read the full transcript.
    session: dict = {
    ...
    tool_functions = create_tools(session)
    ...
    async def on_client_connected(transport, client):
        ...
        context.add_message({"role": "user", "content": GREETING_KICKOFF})
...
async def bot(runner_args: RunnerArguments):
    ...
    match runner_args:
        ...
        case WebSocketRunnerArguments():
            ...
            # Fetch call information from Twilio REST API so we can text the
            # officer the review link at the caller's number.
